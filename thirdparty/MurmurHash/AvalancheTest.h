...
calcBias (pfHash hash, std::vector < int >&counts, int reps, Rand & r)
{
  ...
  bool AvalancheTest (pfHash hash, const int reps)
{
  ...
  printf ("Testing %3d-bit keys -> %3d-bit hashes, %8d reps", keybits,
	  hashbits, reps);
...
template < typename keytype, typename hashtype > void
BicTest (pfHash hash, const int keybit, const int reps, double &maxBias,
  ...
  for (int out1 = 0; out1 < hashbits; out1++)
      ...
      for (int out2 = 0; out2 < hashbits; out2++)
	{
	  ...
	  for (int b = 0; b < 4; b++)
	    {
	      double b2 =
		double (bins[(out1 * hashbits + out2) * 4 + b]) /
		double (reps / 2);
	      b2 = fabs (b2 * 2 - 1);
...
template < typename keytype, typename hashtype > void
BicTest3 (pfHash hash, const int reps, bool verbose = true)
{
  const int keybytes = sizeof (keytype);
  const int keybits = keybytes * 8;
  const int hashbytes = sizeof (hashtype);
...
template < typename keytype, typename hashtype > void
BicTest2 (pfHash hash, const int reps, bool verbose = true)
{
  const int keybytes = sizeof (keytype);
  const int keybits = keybytes * 8;
  const int hashbytes = sizeof (hashtype);
