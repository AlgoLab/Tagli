...
double
CalcMean (std::vector < double >&v, int a, int b)
{
  double mean = 0;
...
double
CalcStdv (std::vector < double >&v, int a, int b)
{
...
bool
ContainsOutlier (std::vector < double >&v, size_t len)
{
  double mean = 0;
...
void
FilterOutliers (std::vector < double >&v)
{
...
void
FilterOutliers2 (std::vector < double >&v)
{
  std::sort (v.begin (), v.end ());
...
NEVER_INLINE int64_t
timehash (pfHash hash, const void *key, int len, int seed)
{
...
void
BulkSpeedTest (pfHash hash, uint32_t seed)
{
...
void
TinySpeedTest (pfHash hash, int hashsize, int keysize, uint32_t seed,
	       bool verbose, double & /*outCycles */ )
{
