...
template < class keytype >
  bool ProcessDifferentials (std::vector < keytype > &diffs, int reps,
			     bool dumpCollisions)
{
...
DiffTestRecurse (pfHash hash, keytype & k1, keytype & k2, hashtype & h1,
		 hashtype & h2, int start, int bitsleft,
  ...
  bool DiffTest (pfHash hash, int diffbits, int reps, bool dumpCollisions)
{
  ...
  for (int i = 0; i < reps; i++)
	...
	printf (".");
      ...
      DiffTestRecurse < keytype, hashtype > (hash, k1, k2, h1, h2, 0,
					     diffbits, diffs);
    }
  printf ("\n");
...
template < typename keytype, typename hashtype > void
DiffDistTest (pfHash hash, const int diffbits, int trials, double &worst,
	      double &avg)
{
