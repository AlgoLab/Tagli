...
uint32_t
MurmurOAAT (const void *key, int len, uint32_t seed)
{
  const uint8_t *data = (const uint8_t *) key;
...
void
FNV (const void *key, int len, uint32_t seed, void *out)
{
  unsigned int h = seed;
...
uint32_t
x17 (const void *key, int len, uint32_t h)
{
  const uint8_t *data = (const uint8_t *) key;
...
#define c8mix( in ) { h *= m; c8fold( in, m, k, h ); }
  ...
  const uint32_t m = 0x83d2e73b, n = 0x97e1cc59, *key4 =
    (const uint32_t *) key;
