...
HashInfo g_hashes[] = {
  ...
  {CityHash128_test, 128, 0x6531F54E, "City128",
   "Google CityHash128WithSeed"},

  {SpookyHash32_test, 32, 0x3F798BBB, "Spooky32",
   "Bob Jenkins' SpookyHash, 32-bit result"},
  {SpookyHash64_test, 64, 0xA7F955F1, "Spooky64",
   "Bob Jenkins' SpookyHash, 64-bit result"},
  {SpookyHash128_test, 128, 0x8D263080, "Spooky128",
   "Bob Jenkins' SpookyHash, 128-bit result"},

  // MurmurHash2

  {MurmurHash2_test, 32, 0x27864C1E, "Murmur2",
   "MurmurHash2 for x86, 32-bit"},
  {MurmurHash2A_test, 32, 0x7FBD4396, "Murmur2A",
   "MurmurHash2A for x86, 32-bit"},
  {MurmurHash64A_test, 64, 0x1F0D3804, "Murmur2B",
   "MurmurHash2 for x64, 64-bit"},
  {MurmurHash64B_test, 64, 0xDD537C05, "Murmur2C",
  ...
  {PMurHash32_test, 32, 0xB0F57EE3, "PMurHash32",
  ...
  for (size_t i = 0; i < sizeof (g_hashes) / sizeof (HashInfo); i++)
    {
      ...
      pass &=
	VerificationTest (info->hash, info->hashbits, info->verification,
			  false);
    }
...
template < typename hashtype > void
test (hashfunc < hashtype > hash, HashInfo * info)
{
  ...
  if (g_testDiff || g_testAll)
    {
      ...
      result &=
	DiffTest < Blob < 64 >, hashtype > (hash, 5, 1000, dumpCollisions);
      result &=
	DiffTest < Blob < 128 >, hashtype > (hash, 4, 1000, dumpCollisions);
      result &=
	DiffTest < Blob < 256 >, hashtype > (hash, 3, 1000, dumpCollisions);
  ...
  if (g_testCyclic || g_testAll)
    {
      ...
      result &=
	CyclicKeyTest < hashtype > (hash, sizeof (hashtype) + 0, 8, 10000000,
				    drawDiagram);
      result &=
	CyclicKeyTest < hashtype > (hash, sizeof (hashtype) + 1, 8, 10000000,
				    drawDiagram);
      result &=
	CyclicKeyTest < hashtype > (hash, sizeof (hashtype) + 2, 8, 10000000,
				    drawDiagram);
      result &=
	CyclicKeyTest < hashtype > (hash, sizeof (hashtype) + 3, 8, 10000000,
				    drawDiagram);
      result &=
	CyclicKeyTest < hashtype > (hash, sizeof (hashtype) + 4, 8, 10000000,
				    drawDiagram);
  ...
  if (g_testSparse || g_testAll)
    {
      ...
      result &=
	SparseKeyTest < 32, hashtype > (hash, 6, true, true, true,
					drawDiagram);
      result &=
	SparseKeyTest < 40, hashtype > (hash, 6, true, true, true,
					drawDiagram);
      result &=
	SparseKeyTest < 48, hashtype > (hash, 5, true, true, true,
					drawDiagram);
      result &=
	SparseKeyTest < 56, hashtype > (hash, 5, true, true, true,
					drawDiagram);
      result &=
	SparseKeyTest < 64, hashtype > (hash, 5, true, true, true,
					drawDiagram);
      result &=
	SparseKeyTest < 96, hashtype > (hash, 4, true, true, true,
					drawDiagram);
      result &=
	SparseKeyTest < 256, hashtype > (hash, 3, true, true, true,
					 drawDiagram);
      result &=
	SparseKeyTest < 2048, hashtype > (hash, 2, true, true, true,
					  drawDiagram);
  ...
  if (g_testPermutation || g_testAll)
	...
	result &=
	  CombinationKeyTest < hashtype > (hash, 8, blocks,
					   sizeof (blocks) /
	...
	uint32_t blocks[] = {
	  ...
	  0x20000000, 0x40000000, 0x60000000, 0x80000000, 0xA0000000,
	...
	result &=
	  CombinationKeyTest < hashtype > (hash, 20, blocks,
					   sizeof (blocks) /
					   sizeof (uint32_t), true, true,
					   drawDiagram);
	...
	uint32_t blocks[] = {
	  ...
	  0x80000000, 0x40000000, 0xC0000000, 0x20000000, 0xA0000000,
	    0x60000000, 0xE0000000
	};

	result &=
	  CombinationKeyTest < hashtype > (hash, 6, blocks,
					   sizeof (blocks) /
					   sizeof (uint32_t), true, true,
					   drawDiagram);
  ...
  if (g_testWindow || g_testAll)
    {
      ...
      result &=
	WindowedKeyTest < Blob < hashbits * 2 >, hashtype > (hash, 20,
							     testCollision,
							     testDistribution,
							     drawDiagram);
...
VerifyHash (const void *key, int len, uint32_t seed, void *out)
{
  ...
  g_outputVCode =
    MurmurOAAT (out, g_hashUnderTest->hashbits / 8, g_outputVCode);
}
