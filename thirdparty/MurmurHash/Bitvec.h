...
setbit (T & blob, uint32_t bit)
{
  return setbit (&blob, sizeof (blob), bit);
}

template <> inline void
setbit (uint32_t & blob, uint32_t bit)
{
  blob |= uint32_t (1) << (bit & 31);
}

template <> inline void
setbit (uint64_t & blob, uint32_t bit)
{
...
template < typename T > inline void
flipbit (T & blob, uint32_t bit)
{
  flipbit (&blob, sizeof (blob), bit);
}

template <> inline void
flipbit (uint32_t & blob, uint32_t bit)
{
  bit &= 31;
  blob ^= (uint32_t (1) << bit);
}
...
template <> inline void
flipbit (uint64_t & blob, uint32_t bit)
{
  bit &= 63;
  blob ^= (uint64_t (1) << bit);
}
...
inline void
lshift (void *blob, int len, int c)
{
  if ((len & 3) == 0)
    {
      lshift32 (blob, len, c);
...
inline void
rshift (void *blob, int len, int c)
{
  if ((len & 3) == 0)
    {
      rshift32 (blob, len, c);
...
template <> inline void
lshift (uint32_t & blob, int c)
{
  blob <<= c;
}

template <> inline void
lshift (uint64_t & blob, int c)
{
  blob <<= c;
}

template <> inline void
rshift (uint32_t & blob, int c)
{
  blob >>= c;
}

template <> inline void
rshift (uint64_t & blob, int c)
{
  blob >>= c;
}
...
template <> inline void
lrot (uint32_t & blob, int c)
{
  blob = ROTL32 (blob, c);
}

template <> inline void
lrot (uint64_t & blob, int c)
{
  blob = ROTL64 (blob, c);
}

template <> inline void
rrot (uint32_t & blob, int c)
{
  blob = ROTR32 (blob, c);
}

template <> inline void
rrot (uint64_t & blob, int c)
{
  blob = ROTR64 (blob, c);
}
...
inline uint32_t
window (void *blob, int len, int start, int count)
{
  if (len & 3)
    {
