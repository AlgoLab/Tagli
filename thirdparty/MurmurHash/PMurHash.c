...
void
PMurHash32_Process (uint32_t * ph1, uint32_t * pcarry, const void *key,
		    int len)
{
  uint32_t h1 = *ph1;
  uint32_t c = *pcarry;
...
uint32_t
PMurHash32_Result (uint32_t h, uint32_t carry, uint32_t total_length)
{
  uint32_t k1;
  int n = carry & 3;
  if (n)
...
uint32_t
PMurHash32 (uint32_t seed, const void *key, int len)
{
...
void
PMurHash32_test (const void *key, int len, uint32_t seed, void *out)
{
  uint32_t h1 = seed, carry = 0;
  const uint8_t *ptr = (uint8_t *) key;
