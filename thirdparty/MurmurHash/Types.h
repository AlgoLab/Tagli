...
template < typename hashtype, typename keytype > struct CollisionMap:public
  std::map <
  hashtype,
  std::vector <
keytype > >
...
template < typename hashtype > struct HashSet:
  public
  std::set <
  hashtype >
{
...
hashfunc (pfHash h):m_hash (h)
  ...
  inline void
  operator  () (const void *key, const int len, const uint32_t seed,
		uint32_t * out)
  {
...
struct KeyCallback
{
  ...
  virtual void
  operator () (const void *key, int len)
  {
    m_count++;
  }

  virtual void
  reserve (int keycount)
  {
  };
...
template < typename hashtype > struct HashCallback:
  public
  KeyCallback
{
  typedef
...
template < typename hashtype > struct CollisionCallback:
  public
  KeyCallback
{
  typedef
