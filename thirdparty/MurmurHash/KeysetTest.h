...
bool VerificationTest (pfHash hash, const int hashbits, uint32_t expected,
		       bool verbose);
...
CombinationKeygenRecurse (uint32_t * key, int len, int maxlen,
			  ...
			  pfHash hash, std::vector < hashtype > &hashes)
{
  ...
  for (int i = 0; i < blockcount; i++)
	...
	hashes.push_back (h);
      ...
      //else
      {
	CombinationKeygenRecurse (key, len + 1, maxlen, blocks, blockcount,
				  hash, hashes);
      }
    }
}
...
template < typename hashtype >
  bool CombinationKeyTest (hashfunc < hashtype > hash, int maxlen,
			   uint32_t * blocks, int blockcount, bool testColl,
			   bool testDist, bool drawDiagram)
{
  ...
  CombinationKeygenRecurse < hashtype > (key, 0, maxlen, blocks, blockcount,
					 hash, hashes);
  ...
  result &=
    TestHashList < hashtype > (hashes, testColl, testDist, drawDiagram);
...
template < typename hashtype > void
PermutationKeygenRecurse (pfHash hash, uint32_t * blocks, int blockcount,
			  int k, std::vector < hashtype > &hashes)
{
  if (k == blockcount - 1)
    {
      hashtype h;
...
template < typename hashtype >
  bool PermutationKeyTest (hashfunc < hashtype > hash, uint32_t * blocks,
			   int blockcount, bool testColl, bool testDist,
			   bool drawDiagram)
{
...
template < typename keytype, typename hashtype > void
SparseKeygenRecurse (pfHash hash, int start, int bitsleft, bool inclusive,
		     keytype & k, std::vector < hashtype > &hashes)
{
  ...
  for (int i = start; i < nbits; i++)
      ...
      if (inclusive || (bitsleft == 1))
	{
      ...
      if (bitsleft > 1)
	{
	  SparseKeygenRecurse (hash, i + 1, bitsleft - 1, inclusive, k,
			       hashes);
	}
...
template < int keybits, typename hashtype > bool
SparseKeyTest (hashfunc < hashtype > hash, const int setbits, bool inclusive,
...
template < typename keytype, typename hashtype >
  bool WindowedKeyTest (hashfunc < hashtype > hash, const int windowbits,
			bool testCollision, bool testDistribution,
			bool drawDiagram)
{
  ...
  printf
    ("Keyset 'Windowed' - %3d-bit key, %3d-bit window - %d tests, %d keys per test\n",
  ...
  for (int j = 0; j <= testcount; j++)
      ...
      for (int i = 0; i < keycount; i++)
	{
      ...
      result &=
	TestHashList (hashes, testCollision, testDistribution, drawDiagram);
...
template < typename hashtype >
  bool CyclicKeyTest (pfHash hash, int cycleLen, int cycleReps,
		      const int keycount, bool drawDiagram)
{
...
template < typename hashtype >
  bool TextKeyTest (hashfunc < hashtype > hash, const char *prefix,
		    const char *coreset, const int corelen,
		    const char *suffix, bool drawDiagram)
{
