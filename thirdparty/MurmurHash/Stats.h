...
inline double
ExpectedCollisions (double balls, double bins)
{
...
FindCollisions (std::vector < hashtype > &hashes,
		HashSet < hashtype > &collisions, int maxCollisions)
{
  int collcount = 0;
...
template < class keytype, typename hashtype > int
PrintCollisions (hashfunc < hashtype > hash, std::vector < keytype > &keys)
{
  int collcount = 0;
...
template < typename hashtype > double
TestDistribution (std::vector < hashtype > &hashes, bool drawDiagram)
{
  ...
  printf ("Worst bias is the %3d-bit window at bit %3d - %5.3f%%", worstWidth,
	  worstStart, pct);
  if (pct >= 1.0)
    printf (" !!!!! ");
  printf ("\n");
...
template < typename hashtype >
  bool TestHashList (std::vector < hashtype > &hashes,
		     std::vector < hashtype > &collisions, bool testDist,
		     bool drawDiagram)
{
  bool result = true;

  {
    ...
    double expected =
      (double (count) * double (count - 1)) /pow (2.0,
						  double (sizeof (hashtype) *
							  8 + 1));
...
template < typename hashtype >
  bool TestHashList (std::vector < hashtype > &hashes, bool /*testColl */ ,
...
template < typename hashtype > double
TestDistributionBytepairs (std::vector < hashtype > &hashes, bool drawDiagram)
{
...
TestDistributionFast (std::vector < hashtype > &hashes, double &dworst,
		      double &davg)
{
  const int hashbits = sizeof (hashtype) * 8;
  const int nbins = 65536;
  ...
  for (int start = 0; start < hashbits; start += 8)
      ...
      for (size_t j = 0; j < hashes.size (); j++)
	{
      ...
      double n =
	calcScore (&bins.front (), (int) bins.size (), (int) hashes.size ());
